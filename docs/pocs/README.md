...
| --- | --- | --- |
| **A — Own SVG from chem coords** | Can Indigo/RDKit layouts feed a publication-quality SVG we own (no SVG mutation)? | `uv run python scripts/run_pocs.py svg` |
...
| **C — ELK multi-mol** | Can elkjs place network/reaction viewports via a Node bridge? | `uv run python scripts/run_pocs.py elk` |
