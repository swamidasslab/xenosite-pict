"""Client + drawable cleanup — Python OOP; Rust owns molecule ink.

Kept: coord normalization, display labels, annotation/caption drawables.
``label_place`` stays in Python. Bond / shade / mark painting is Rust
``depict_molecule`` (no obsolete ``return None`` drawables).
"""
...
class Drawable(ABC):
    """Overlay painter (annotations / captions). Molecule ink is Rust."""
...
def molecule_drawables(mol_spec: MoleculeSpec) -> list[Drawable]:
    ...
    out: list[Drawable] = [
        AnnotationDrawable(ann) for ann in ring_attachment_annotations(mol_spec)
    ]
    out.extend(AnnotationDrawable(ann) for ann in mol_spec.annotations)
